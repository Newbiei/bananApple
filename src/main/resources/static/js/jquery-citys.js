 ...
 *==================================*/
+function($) {
	$.fn.getAreaId = function(option) {
		var areaId = $('#' + option + ' select[name="area"]').val();		// 获取区
		if (areaId == '- 请选择 -' || $('#' + option + ' select[name="area"] option').length == 0) {	// 区没选或区列表为空
			areaId = $('#' + option + ' select[name="city"]').val();		// 获取市
		}
		if (areaId == '- 请选择 -') {								// 市没选
			areaId = $('#' + option + ' select[name="province"]').val();	// 获取省
		}
		if (areaId == '- 请选择 -') {								// 省没选
			areaId = '';											// 置空
		}
